...
class EmailList extends TestCase
{
    ...
    {
        //Set up an email
        $email = factory(\App\Models\EmailList::class)->create([
            'subscribe' => true
        ]);

        //Make sure the email is subscribed
        $this->assertEquals(1, $email->subscribe);
        $this->assertEquals(1, \App\Models\EmailList::subscribed()->count());
        $this->assertEquals(0, \App\Models\EmailList::unsubscribed()->count());
        ...
        $this->assertEquals(0, \App\Models\EmailList::subscribed()->count());
        $this->assertEquals(1, \App\Models\EmailList::unsubscribed()->count());
    }

    /** @test */
    public function a_user_should_be_able_to_subscribe_by_filling_out_the_sing_up_for_the_news_letter_form()
    {
        $this->assertEquals(0, \App\Models\EmailList::all()->count());
        ...
        $this->assertEquals(1, \App\Models\EmailList::all()->count());
        $this->assertEquals(1, \App\Models\EmailList::subscribed()->count());
